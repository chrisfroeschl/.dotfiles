...
var ScriptConfig = function (options, identifier) {
  if (!options) throw "Options table parameter is missing.";
  this.options = options;
  this.scriptName =
    typeof identifier === "string" ? identifier : mp.get_script_name();
  ...
  var typeConv = function (destTypeVal, val) {
    switch (typeof destTypeVal) {
      case "object":
        if (!Array.isArray(destTypeVal)) val = undefined;
        // Unknown "object" target variable.
        else if (typeof val !== "string") val = String(val); // Target is array, so use string values.
        break;
      case "string":
      ...
      case "boolean":
        if (val === "yes") val = true;
          ...
          mp.msg.error(
            "Error: Can't convert " + JSON.stringify(val) + " to boolean!"
          );
          val = undefined;
        }
        break;
      case "number":
        ...
        if (!isNaN(num)) val = num;
          ...
          mp.msg.error(
            "Error: Can't convert " + JSON.stringify(val) + " to number!"
          );
          val = undefined;
        }
        break;
  ...
  if (this.scriptName && this.scriptName.length) {
    ...
    this.configFile = mp.find_config_file(
      "script-settings/" + this.scriptName + ".conf"
    );
    if (!this.configFile)
      // Try legacy settings location as fallback.
      this.configFile = mp.find_config_file(
        "lua-settings/" + this.scriptName + ".conf"
      );
  }
  ...
  if (this.configFile && this.configFile.length) {
    try {
      var line,
        configLines = mp.utils.read_file(this.configFile).split(/[\r\n]+/);
      for (i = 0, len = configLines.length; i < len; ++i) {
        ...
        if (pos < 0) {
          mp.msg.warn(
            '"' +
              this.configFile +
              '": Ignoring malformatted config line "' +
              line.replace(/\s+$/, "") +
              '".'
          );
          continue;
        ...
        if (this.options.hasOwnProperty(key)) {
          ...
          if (typeof convVal !== "undefined") {
            if (Array.isArray(this.options[key])) {
              if (isArrayVal) this.options[key].push(convVal);
              else
                mp.msg.error(
                  '"' +
                    this.configFile +
                    '": Ignoring non-array value for array-based option key "' +
                    key +
                    '".'
                );
            } else this.options[key] = convVal;
          } else
            mp.msg.error(
              '"' +
                this.configFile +
          ...
          mp.msg.warn(
            '"' + this.configFile + '": Ignoring unknown key "' + key + '".'
          );
      }
    } catch (e) {
      mp.msg.error(
        'Unable to read configuration file "' + this.configFile + '".'
      );
    }
  } else
    mp.msg.verbose(
      "Unable to find configuration file for " + this.scriptName + "."
    );

  // Parse command-line options.
  if (this.scriptName && this.scriptName.length) {
    var cmdOpts = mp.get_property_native("options/script-opts"),
      rawOpt,
      prefix = this.scriptName + "-",
      keyLen;
    len = prefix.length;
    for (rawOpt in cmdOpts) {
      if (!cmdOpts.hasOwnProperty(rawOpt)) continue;
        ...
        if (typeof convVal !== "undefined") {
          if (Array.isArray(this.options[key])) {
            if (isArrayVal) this.options[key].push(convVal);
              ...
              mp.msg.error(
                'script-opts: Ignoring non-array value for array-based option key "' +
                  key +
                  '".'
              );
          } else this.options[key] = convVal;
          ...
          mp.msg.error(
            'script-opts: Unable to convert value "' +
              val +
              '" for key "' +
              key +
              '".'
          );
      } else mp.msg.warn('script-opts: Ignoring unknown key "' + key + '".');
    }
  }
};

ScriptConfig.prototype.getValue = function (key) {
  if (!this.options.hasOwnProperty(key)) throw 'Invalid option "' + key + '"';
...
ScriptConfig.prototype.getMultiValue = function (key) {
  ...
  if (len) {
    if (val.charAt(0) !== "{" || val.charAt(len - 1) !== "}")
      throw (
        'Missing surrounding "{}" brackets in multi-value option "' + key + '"'
      );
