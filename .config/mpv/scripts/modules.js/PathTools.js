...
PathTools.getCwd = function (strictErrors) {
  ...
  if (strictErrors) throw mp.last_error();
  return "";
};
...
PathTools._detectOS = function () {
  ...
  if (PathTools._isUnix) {
    ...
    if (
      typeof unameResult.stdout === "string" &&
      unameResult.stdout.match(/^\s*Darwin\s*$/)
    )
...
PathTools.isUnix = function () {
  if (PathTools._isUnix === null) PathTools._detectOS();
  return PathTools._isUnix;
};

PathTools.isMac = function () {
  if (PathTools._isMac === null) PathTools._detectOS();
  return PathTools._isMac;
};

PathTools.pathSep = function () {
  if (PathTools._pathSep === null) PathTools._detectOS();
...
PathTools.getPathInfo = function (path) {
  ...
  try {
    var dirContents = new PathIndex(path, {
      // Throws.
...
PathTools.getParentPath = function (path) {
  ...
  if (pathParts.length > 1)
    // Refuse to remove last remaining (drive root).
    previousDir = pathParts.pop();
  var newPath = pathParts.join(PathTools._pathSep);
  if (PathTools._isUnix && !newPath.length)
    // Preserve unix drive root.
    newPath = "/";
  if (!newPath.length)
    // Safeguard against empty parent path result.
    newPath = path;
  return {
...
PathTools.getSubPath = function (path, file) {
  if (PathTools._isUnix === null || PathTools._pathSep === null)
    PathTools._detectOS();
  return (
    (PathTools._isUnix && path === "/" ? "/" : path + PathTools._pathSep) + file
  );
};
...
PathTools.makePathAbsolute = function (path) {
  return PathTools.isPathAbsolute(path)
    ? path
